...
import {
  ROLES, TASK_TYPE_KEYS, canViewTask, canViewField, canPlanTask,
...
const STATUS_ORDER: TaskStatus[] = ['scheduled', 'in_progress', 'completed', 'missed'];
...
export function SchedulePage() {
  ...
  const fieldName = (id: string) => {
    ...
    return field ? t(field.shortKey) : id;
  };
  const user = useUser();
  ...
  const calendarDays = useMemo(() => {
    const start = startOfWeek(selectedDate, { locale: dateLocale });
  ...
  const accessNote = isContractor
    ? t('schedule.hintContractor', { company: user.companyKey ? t(user.companyKey) : '' })
    : level === 'field'
      ? t('schedule.hintField', {
          role: t(ROLES[user.role].labelKey),
          fields: visibleFields.map(f => fieldName(f.id)).join(', '),
        })
      : level === 'specialist'
        ? t('schedule.hintSpecialist', {
            role: t(ROLES[user.role].labelKey),
            types: plannableTypes.map(type => t(TASK_TYPE_KEYS[type])).join(', '),
          })
        : t('schedule.hintFull');
    ...
    <div className="h-full flex flex-col bg-[#f5f5f7]">
      ...
      <div className="px-4 sm:px-8 py-5 sm:py-8 bg-white border-b border-black/5 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div className="flex flex-col gap-1 min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-[#1d1d1f]">{isContractor ? t('schedule.titleContractor') : t('schedule.title')}</h1>
          <p className="text-[12px] sm:text-[13px] text-[#6e6e73] font-medium flex items-center gap-2">
            <CalendarIcon className="w-4 h-4 flex-shrink-0" />
            {t('schedule.todayIs', { date: format(new Date(), 'd MMMM yyyy', { locale: dateLocale }) })}
          </p>
        ...
        {plannableTypes.length > 0 && (
          ...
          >
            <Plus className="w-4 h-4" /> {t('schedule.createTask')}
          </button>
        )}
      </div>
         ...
         <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center justify-between mb-4 px-2">
               <h3 className="text-[13px] font-bold uppercase tracking-[0.2em] text-[#6e6e73]">
                  {isDateToday(selectedDate) ? t('schedule.planToday') : format(selectedDate, 'd MMMM', { locale: dateLocale })}
               </h3>
               <span className="text-[11px] font-bold text-[#86868b]">{t('schedule.tasks', { count: dailyTasks.length })}</span>
            </div>
               ...
               {dailyTasks.length === 0 ? (
                 ...
                 >
                    ...
                    <p className="text-[14px] font-medium text-[#6e6e73]">
                      {isContractor ? t('schedule.noTasksContractor') : t('schedule.noTasks')}
                    </p>
                 </motion.div>
               ) : (
      ...
      <AddTaskModal isOpen={showAddModal} types={plannableTypes} fieldOptions={visibleFields}
        defaultFieldId={fieldFilter ?? visibleFields[0]?.id} managerName={t(user.nameKey)}
        onClose={() => setShowAddModal(false)}
        onAdd={task => { setTasks(prev => [...prev, task]); setShowAddModal(false); setSelectedDate(startOfDay(task.date)); }}
      />
    </div>
  );
}
...
function TaskCard({ task, index, onUpdate, onDelete }: {
  ...
  const startEdit = (kind: 'fact' | 'plan') => {
    setDraft(formatAmount(t, kind === 'fact' ? (task.factVolume ?? task.volume) : task.volume));
    setEditing(kind);
  };
  const saveEdit = () => {
    ...
    >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className={`text-[13px] sm:text-[15px] font-bold ${done ? 'line-through text-[#6e6e73]' : ''}`}>
              {t(TASK_TYPE_KEYS[task.type])} · {t(task.sector)}
            </span>
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-[#f5f5f7] text-[#6e6e73] uppercase tracking-wider">{t(task.crop)}</span>
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider ${STATUS_TINT[task.status]}`}>{t(`taskStatus.${task.status}`)}</span>
          </div>
          <div className="flex items-center gap-x-4 gap-y-1 text-[11px] sm:text-[12px] font-medium text-[#6e6e73] flex-wrap">
            <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> {format(task.date, 'HH:mm')} • {task.duration} {t('common.min')}</span>
            <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> {field ? t(field.shortKey) : ''} · {field?.areaHectares} {t('common.ha')}</span>
            <span>{t('schedule.norm')} <b className="text-[#1d1d1f]">{formatAmount(t, task.volume)}</b></span>
            {task.factVolume && <span>{t('schedule.fact')} <b className="text-green-600">{formatAmount(t, task.factVolume)}</b></span>}
          </div>
          <div className="flex items-center gap-x-4 gap-y-1 text-[11px] font-medium text-[#86868b] mt-1 flex-wrap">
            <span className="flex items-center gap-1.5"><UserRound className="w-3.5 h-3.5" /> {t('schedule.assignee')} {assignee?.companyKey ? t(assignee.companyKey) : t('schedule.notAssigned')}</span>
            <span>{t('schedule.planner')} {t(task.managerName)}</span>
          ...
          {editing ? (
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <span className="text-[11px] font-bold text-[#6e6e73] uppercase tracking-wider flex-shrink-0">
                {editing === 'fact' ? t('schedule.factShort') : t('schedule.normShort')}
              </span>
            ...
            <>
              {canOperate && (
                <>
                  <select value={task.assigneeId ?? ''} onChange={e => onUpdate({ assigneeId: e.target.value || undefined })}
                    className={selectCls} aria-label={t('schedule.assigneeAria')}
                  >
                    <option value="">{t('schedule.noAssignee')}</option>
                    {contractors.map(c => <option key={c.id} value={c.id}>{c.companyKey ? t(c.companyKey) : t(c.nameKey)}</option>)}
                  </select>
                  <select value={task.status} onChange={e => onUpdate({ status: e.target.value as TaskStatus })}
                    className={selectCls} aria-label={t('schedule.statusAria')}
                  >
                    {STATUS_ORDER.map(s => <option key={s} value={s}>{t(`taskStatus.${s}`)}</option>)}
                  </select>
                  <button onClick={() => startEdit('fact')} className={`${actionBtn} bg-[#f5f5f7] text-[#1d1d1f] hover:bg-black/10`}>
                    <Pencil className="w-3.5 h-3.5" /> {t('schedule.enterFact')}
                  </button>
                </>
              )}
              {canPlan && (
                <>
                  <button onClick={() => startEdit('plan')} className={`${actionBtn} bg-[#f5f5f7] text-[#1d1d1f] hover:bg-black/10`}>
                    <Pencil className="w-3.5 h-3.5" /> {t('schedule.changeNorm')}
                  </button>
                  <button onClick={onDelete} className={`${actionBtn} text-red-500 hover:bg-red-50`} aria-label={t('schedule.deleteTask')}>
              ...
              {canConfirm && (
                <>
                  <button onClick={() => startEdit('fact')} className={`${actionBtn} bg-[#f5f5f7] text-[#1d1d1f] hover:bg-black/10`}>
                    <Pencil className="w-3.5 h-3.5" /> {t('schedule.setFact')}
                  </button>
                  ...
                  >
                    <CheckCircle2 className="w-4 h-4" /> {t('schedule.confirmDone')}
                  </button>
                </>
              )}
            </>
      ...
      {task.confirmedAt && (
        <p className="mt-2 text-[11px] font-semibold text-green-600 flex items-center gap-1.5">
          <CheckCircle2 className="w-3.5 h-3.5" /> {t('schedule.confirmedAt', { date: format(task.confirmedAt, 'd MMM, HH:mm', { locale: dateLocale }) })}
        </p>
      )}
    </motion.div>
  );
...
function AddTaskModal({ isOpen, types, fieldOptions, defaultFieldId, managerName, onClose, onAdd }: {
  ...
  const [form, setForm] = useState({
    ...
    sector: t('data.sectors.zoneA'), crop: t('data.crops.wheat'),
    type: types[0] as TaskType | undefined,
    volume: `15 ${t('unit.lm2')}`, duration: 40,
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4 sm:p-6" onClick={onClose}>
          ...
          >
            <div className="flex items-center justify-between mb-6 sm:mb-8">
              <h2 className="text-lg sm:text-xl font-bold">{t('schedule.newOperation')}</h2>
              <button onClick={onClose} className="p-2 hover:bg-black/5 rounded-lg"><X className="w-5 h-5 text-[#6e6e73]" /></button>
            </div>

            <div className="space-y-6">
               <div>
                  <label className={`${labelCls} mb-2`}>{t('schedule.impactType')}</label>
                  <div className={`grid gap-3 ${types.length > 1 ? 'grid-cols-3' : 'grid-cols-1'}`}>
                     {types.map(kind => {
                       const { icon: Icon } = TYPE_STYLE[kind];
                       return (
                         <button key={kind} onClick={() => setForm(f => ({ ...f, type: kind }))}
                           className={`flex items-center justify-center gap-2 py-3 rounded-xl text-[13px] font-bold border transition-all ${type === kind ? 'bg-[#0071e3] text-white border-blue-500 shadow-lg shadow-blue-500/20' : 'bg-white border-black/10 text-[#6e6e73]'}`}
                         >
                           <Icon className="w-4 h-4" /> {t(TASK_TYPE_KEYS[kind])}
                         </button>
                       );
                     })}
                  </div>
               ...
               <div className="space-y-2">
                  <label className={labelCls}>{t('schedule.fieldLabel')}</label>
                  <select value={fieldId} onChange={e => setForm(f => ({ ...f, fieldId: e.target.value }))} className={inputCls}>
                    {fieldOptions.map(f => <option key={f.id} value={f.id}>{t(f.nameKey)}</option>)}
                  </select>
               </div>
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                     <label className={labelCls}>{t('schedule.sectorLabel')}</label>
                     <input value={form.sector} onChange={e => setForm(f => ({ ...f, sector: e.target.value }))} className={inputCls} />
                  </div>
                  <div className="space-y-2">
                     <label className={labelCls}>{t('schedule.cropLabel')}</label>
                     <input value={form.crop} onChange={e => setForm(f => ({ ...f, crop: e.target.value }))} className={inputCls} />
                  </div>
                  <div className="space-y-2">
                     <label className={labelCls}>{t('schedule.normLabel')}</label>
                     <input value={form.volume} onChange={e => setForm(f => ({ ...f, volume: e.target.value }))} className={inputCls} />
                  </div>
                  <div className="space-y-2">
                     <label className={labelCls}>{t('schedule.durationLabel')}</label>
                     <input type="number" min={1} value={form.duration} onChange={e => setForm(f => ({ ...f, duration: Number(e.target.value) }))} className={inputCls} />
                  </div>
               </div>
               <div className="space-y-2">
                  <label className={labelCls}>{t('schedule.dateTimeLabel')}</label>
                  <input type="datetime-local" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} className={inputCls} />
               </div>
               <button onClick={handleSubmit} className="w-full py-4 bg-[#0071e3] text-white rounded-2xl text-[15px] font-bold shadow-lg shadow-blue-500/20 hover:bg-[#0077ed] transition-all">{t('schedule.saveToPlan')}</button>
            </div>
          </motion.div>
        </div>
      )}
